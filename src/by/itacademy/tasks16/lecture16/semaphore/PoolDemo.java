package cuncurrency.semaphore;

import by.itacademy.tasks16.lecture16.semaphore.Pool;
        ...
        while (count ++ < ITERATION_COUNT) {
            ...
            try {
                ...
                switch (choice) {
                    case 1 :
                        pool.putItem(data);
                        System.out.printf("%s Iteration = %d  only get item = %d", Thread.currentThread().getName(),
                                count, data.getData());
                        System.out.println();
                        break;
                    case 2:
                        ...
                        data.setData(putted);
                        pool.putItem(data);
                        System.out.printf("%s Iteration = %d  get and replace on new item = %d",
                                Thread.currentThread().getName(), count, putted);
                        System.out.println();
                        break;
                    default:
